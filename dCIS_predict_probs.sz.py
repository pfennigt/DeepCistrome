...
# The threshold argument is no longer used for output but can be kept for other purposes if needed.
parser.add_argument(
    "--threshold",
    type=float,
...
try:
    ...
    # Create results DataFrame using the raw probability scores
    predictions_df = pd.DataFrame(predictions_raw, columns=tf_names_list)
    
    print(f"  Predictions shape: {predictions_df.shape}")

    # Verify prediction output size against the hardcoded list size
    if predictions_df.shape[0] != len(seq_full_headers):
        print(f"  Error: Number of predictions ({predictions_df.shape[0]}) does not match number of valid sequences ({len(seq_full_headers)}). Internal error.", file=sys.stderr)
        sys.exit(1)

    if predictions_df.shape[1] != len(tf_names_list):
         print(f"\n--- ERROR ---", file=sys.stderr)
         print(f"Mismatch between model output size and hardcoded TF list size:", file=sys.stderr)
         print(f"  Model produced {predictions_df.shape[1]} outputs per sequence.", file=sys.stderr)
